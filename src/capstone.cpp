...
static void BS_HOOK_DLOPEN init_capstone() {
    cs_err e1 = cs_open(CS_ARCH_ARM64, CS_MODE_ARM, &handle);
    cs_option(handle, CS_OPT_DETAIL, 1);
    if (e1) {
        __android_log_print(ANDROID_LOG_FATAL, "[" MOD_ID "|" VERSION "] capstone", "Capstone initialization failed! %u", e1);
        SAFE_ABORT();
    }
    __android_log_print(ANDROID_LOG_INFO, "[" MOD_ID "|" VERSION "] capstone", "Capstone initialized!");
    valid = true;
}
...
uint32_t* cs::readb(uint32_t const* addr) {
    cs_insn* insns;
    // Read from addr, 1 instruction, with pc at addr, into insns.
    // TODO: consider using cs_disasm_iter
    auto count = cs_disasm(handle, reinterpret_cast<uint8_t const*>(addr), sizeof(uint32_t), reinterpret_cast<uint64_t>(addr), 1, &insns);
...
std::optional<uint32_t*> cs::bl_conv(cs_insn* insn) {
...
std::optional<uint32_t*> cs::b_conv(cs_insn* insn) {
...
std::optional<std::tuple<uint32_t*, arm64_reg, uint32_t*>> cs::pc_rel_conv(cs_insn* insn) {
    ...
    switch (insn->id) {
        ...
        case ARM64_INS_ADRP:
            ...
            return tup{
                reinterpret_cast<uint32_t*>(insn->address),
                insn->detail->arm64.operands[0].reg,
...
std::optional<std::tuple<uint32_t*, arm64_reg, int64_t>> cs::reg_match_conv(cs_insn* match, arm64_reg to_match) {
