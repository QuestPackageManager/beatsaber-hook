...
#warning "Include rapidjson.hpp before any rapidjson includes!"
#endif
#else
...
namespace json {
    enum class Endianness { Big, Little, Default, Network = Big };

    template <typename T>
    struct sfinae_fail {};

    template <typename, Endianness = Endianness::Default, bool BOM = false>
    struct encoding_for_char_t;
    ...
    struct encoding_for_char_t<Ch, E, false> {
        ...
        using type = std::conditional<
            size == 1,
            rapidjson::UTF8<Ch>,
            std::conditional<size == 2, rapidjson::UTF16<Ch>, std::conditional<size == 4, rapidjson::UTF32<Ch>, sfinae_fail<Ch>>>>;
    };
    ...
    struct encoding_for_char_t<Ch, Endianness::Big, true> {
        ...
        using type = std::conditional<
            size == 1,
            rapidjson::UTF8<Ch>,
            std::conditional<size == 2, rapidjson::UTF16BE<Ch>, std::conditional<size == 4, rapidjson::UTF32BE<Ch>, sfinae_fail<Ch>>>>;
    };
    template <typename Ch>
    struct encoding_for_char_t<Ch, Endianness::Little, true> {
        ...
        using type = std::conditional<
            size == 1,
            rapidjson::UTF8<Ch>,
            std::conditional<size == 2, rapidjson::UTF16LE<Ch>, std::conditional<size == 4, rapidjson::UTF32LE<Ch>, sfinae_fail<Ch>>>>;
    };
