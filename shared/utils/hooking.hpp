...
#define PARAM(i) ::Hooking::param_get<i>(__args...)
...
struct HookCatchWrapper<Func, R (*)(TArgs...)> {
    static R wrapper(TArgs... args) {
        IL2CPP_CATCH_HANDLER(return Func(args...);)
    }
};
...
#define MAKE_HOOK_FIND_VERBOSE(name_, infoGet, retval, ...)                        \
    struct Hook_##name_ {                                                          \
        ...
        static const MethodInfo* getInfo() {                                       \
            return infoGet;                                                        \
        }                                                                          \
        using funcType = retval (*)(__VA_ARGS__);                                  \
        static funcType* trampoline() {                                            \
...
#define MAKE_HOOK_FIND(name_, klass, mName, retval, ...)                           \
    struct Hook_##name_ {                                                          \
        ...
        static const MethodInfo* getInfo() {                                       \
            return ::il2cpp_utils::MethodTypeCheck<funcType>::find(klass, mName);  \
        }                                                                          \
        using funcType = retval (*)(__VA_ARGS__);                                  \
        static funcType* trampoline() {                                            \
            return &name_;                                                         \
...
#define MAKE_HOOK_FIND_CLASS(name_, namespaze, klassName, mName, retval, ...)                    \
    struct Hook_##name_ {                                                                        \
        ...
        static const MethodInfo* getInfo() {                                                     \
            return ::il2cpp_utils::MethodTypeCheck<funcType>::find(namespaze, klassName, mName); \
        }                                                                                        \
        using funcType = retval (*)(__VA_ARGS__);                                                \
        static funcType* trampoline() {                                                          \
            return &name_;                                                                       \
...
#define MAKE_HOOK_FIND_INSTANCE(name_, klass, mName, retval, ...)                                                                          \
    struct Hook_##name_ {                                                                                                                  \
        ...
        static const MethodInfo* getInfo() {                                                                                               \
            return ::il2cpp_utils::MethodTypeCheck<typename ::il2cpp_utils::InstanceMethodConverter<funcType>::fType>::find(klass, mName); \
        }                                                                                                                                  \
        static funcType* trampoline() {                                                                                                    \
            return &name_;                                                                                                                 \
        }                                                                                                                                  \
...
#define MAKE_HOOK_FIND_CLASS_INSTANCE(name_, namespaze, klassName, mName, retval, ...)                                                                    \
    struct Hook_##name_ {                                                                                                                                 \
        ...
        static const MethodInfo* getInfo() {                                                                                                              \
            return ::il2cpp_utils::MethodTypeCheck<typename ::il2cpp_utils::InstanceMethodConverter<funcType>::fType>::find(namespaze, klassName, mName); \
        }                                                                                                                                                 \
        static funcType* trampoline() {                                                                                                                   \
            return &name_;                                                                                                                                \
        }                                                                                                                                                 \
...
#define MAKE_HOOK_FIND_CLASS_UNSAFE_INSTANCE(name_, namespaze, klassName, mName, retval, ...)                 \
    struct Hook_##name_ {                                                                                     \
        ...
        static const MethodInfo* getInfo() {                                                                  \
            return ::il2cpp_utils::MethodTypeCheck<funcType>::find_unsafe(namespaze, klassName, mName, true); \
        }                                                                                                     \
        using funcType = retval (*)(__VA_ARGS__);                                                             \
        static funcType* trampoline() {                                                                       \
            return &name_;                                                                                    \
...
#define MAKE_HOOK_FIND_CLASS_UNSAFE_STATIC(name_, namespaze, klassName, mName, retval, ...)             \
    struct Hook_##name_ {                                                                               \
        ...
        static const MethodInfo* getInfo() {                                                            \
            return ::il2cpp_utils::MethodTypeCheck<funcType>::find_unsafe(namespaze, klassName, mName); \
        }                                                                                               \
        using funcType = retval (*)(__VA_ARGS__);                                                       \
        static funcType* trampoline() {                                                                 \
            return &name_;                                                                              \
...
#define MAKE_HOOK_CHECKED_INSTANCE_FIND(name_, mPtr, mName, retval, ...)                                                                            \
    struct Hook_##name_ {                                                                                                                           \
        ...
        static const MethodInfo* getInfo() {                                                                                                        \
            return ::il2cpp_utils::MethodTypeCheck<funcType>::find(classof(classType), mName);                                                      \
        }                                                                                                                                           \
        static funcType* trampoline() {                                                                                                             \
            return &name_;                                                                                                                          \
        }                                                                                                                                           \
...
#define MAKE_HOOK_WRAPPER(name_, mPtr, retval, ...)                                                                                                       \
    struct Hook_##name_ {                                                                                                                                 \
        ...
        static funcType hook() {                                                                                                                          \
            return &::Hooking::HookWrapperCompose<::Hooking::HookCatchWrapper<&hook_##name_, decltype(&hook_##name_)>::wrapper>::wrapper;                 \
        }                                                                                                                                                 \
    };                                                                                                                                                    \
    retval Hook_##name_::hook_##name_(__VA_ARGS__)
...
void __InstallFinalHook(L& logger, void* addr) {
    ...
    auto install_result =
        flamingo::Install(flamingo::HookInfo{ (void*)T::hook(), addr, (void**)T::trampoline(), flamingo::HookNameMetadata{ .name = T::name() }, flamingo::HookPriority{ .is_final = true } });
    if (install_result.has_value()) {
// TODO: Attach the returned handle to a member on T.
#ifndef SUPPRESS_MACRO_LOGS
...
void InstallOrigHook(L& logger) {
    ...
    auto addr = (void*)info->methodPointer;
    return __InstallFinalHook<T>(logger, addr);
}
